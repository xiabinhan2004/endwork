/*
 * Created by JFormDesigner on Mon May 31 18:09:57 CST 2021
 */
...
public class UpdateStaff extends JDialog {
    ...
    public UpdateStaff(Window owner, int staId) {
        super(owner);
        this.staId = staId;
        initComponents();
        setVisible(true);
    ...
    private void ok(KeyEvent e) {
        if (e.getKeyChar() == KeyEvent.VK_ENTER){
            ...
            int i = adminMapper.adminUpdateStaff(map);
            if (i > 0){
                System.out.println("adminUpdateStaff Successfully!");
                JOptionPane.showMessageDialog(null, "修改员工信息成功！", "Info", JOptionPane.INFORMATION_MESSAGE);
                this.dispose();
            }
        }
    }
